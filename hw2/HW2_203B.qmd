...
#Since there are very few hospital stays that are longer than 40 hours, I limit the length of x-axis to 40 hours make the graph looks better.
...
lab_icuonly1%>%
#There are many abnormal values on some of the variables, possibly due to the recording errors of the hospital, limit each variable to make the histograms better:
...
ethnicity_group <- mimic_icu_cohort %>%
  ...
  mutate(var_name = str_sub(ethnicity_group$ethnicity, end = 15)) %>%
  print(width = Inf)
```
...
for (i in demo_vars) {
  ...
  p <- ggplot(df) +
    ...
    ggtitle("30-Day-Mortality in Different", paste(i)) +
...
ggarrange(plotlist = plots, nrow = 2, ncol = 3, widths = c(3, 3, 3))
```
