...
logit_recipe <- 
  ...
  # Classify the missing values of marital_status as another
  # level (4617 NAs)
...
```{r}
# Numerical summaries stratified by the outcome `thirty_year_mort`.
icu_cohort_hw4 %>% tbl_summary(by = thirty_day_mort)
```
...
Set cross-validation partitions, use a different fold.
```{r}
set.seed(205)
